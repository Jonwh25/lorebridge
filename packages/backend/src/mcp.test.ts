...
test("MCP endpoint requires pairing and exposes live Foundry tools", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["end_combat", "next_turn", "set_initiative", "get_world_summary", "search_campaign", "search_journals", "get_journal_page", "search_actors", "get_actor", "search_scenes", "get_scene", "get_combat_state", "roll_dice", "get_chat_messages", "search_assets", "get_active_scene", "resolve_uuid", "get_related_documents", "search_items", "get_actor_inventory", "search_session_logs", "get_session_log", "list_compendiums", "search_compendium", "get_compendium_entry", "get_item", "list_folders", "browse_folder", "propose_journal_update", "get_quest_objectives", "propose_quest_objectives_update", "generate_roll_table", "search_roll_tables", "list_playlists", "search_playlists", "list_macro_tools", "list_macros", "call_macro_tool", "list_backup_commits", "check_campaign_health", "audit_campaign_consistency", "read_backup_file", "generate_npc", "create_actor", "update_actor", "generate_item", "create_item", "update_item", "generate_encounter", "create_encounter", "update_scene", "search_campaign_semantic", "rebuild_semantic_index", "generate_npc_dossier_roleplay", "generate_npc_dossier_overview", "generate_npc_dossier_knowledge", "generate_faction", "manage_campaign_codex"],
    );
    const readOnlyTools = tools.tools.filter((t) => t.name !== "end_combat" && t.name !== "next_turn" && t.name !== "set_initiative" && t.name !== "propose_journal_update" && t.name !== "propose_quest_objectives_update" && t.name !== "generate_roll_table" && t.name !== "roll_dice" && t.name !== "call_macro_tool" && t.name !== "create_actor" && t.name !== "update_actor" && t.name !== "create_item" && t.name !== "update_item" && t.name !== "create_encounter" && t.name !== "update_scene" && t.name !== "rebuild_semantic_index" && t.name !== "generate_npc_dossier_roleplay" && t.name !== "generate_npc_dossier_overview" && t.name !== "generate_npc_dossier_knowledge" && t.name !== "generate_faction" && t.name !== "manage_campaign_codex");
    ...
    // Semantic tools return a structured error when no embedding provider is configured.
    const semanticSearchResult = await client.callTool({
      name: "search_campaign_semantic",
      arguments: { query: "vampire lord of Barovia", sourceId: "foundry:cos" },
    });
    assert.equal(semanticSearchResult.isError, true);
    const semanticSearchError = JSON.parse((semanticSearchResult.content as Array<{ type: string; text: string }>)[0]!.text);
    assert.ok(typeof semanticSearchError.error === "string");

    const rebuildResult = await client.callTool({
      name: "rebuild_semantic_index",
      arguments: { sourceId: "foundry:cos" },
    });
    assert.equal(rebuildResult.isError, true);
    const rebuildError = JSON.parse((rebuildResult.content as Array<{ type: string; text: string }>)[0]!.text);
    assert.ok(typeof rebuildError.error === "string");

    // Verify annotations on semantic tools.
    const semanticSearchAnnotations = tools.tools.find((t) => t.name === "search_campaign_semantic")?.annotations;
    assert.equal(semanticSearchAnnotations?.readOnlyHint, true);
    const rebuildAnnotations = tools.tools.find((t) => t.name === "rebuild_semantic_index")?.annotations;
    assert.equal(rebuildAnnotations?.readOnlyHint, false);
    assert.equal(rebuildAnnotations?.destructiveHint, false);
  } finally {
    await client?.close();
    webSocket?.close();
