...
test("MCP endpoint requires pairing and exposes live Foundry tools", async () => {
  ...
  try {
    ...
    const readOnlyTools = tools.tools.filter((t) => t.name !== "end_combat" && t.name !== "next_turn" && t.name !== "set_initiative" && t.name !== "propose_journal_update" && t.name !== "generate_roll_table" && t.name !== "roll_dice" && t.name !== "call_macro_tool" && t.name !== "create_actor" && t.name !== "update_actor" && t.name !== "create_item" && t.name !== "update_item" && t.name !== "create_encounter" && t.name !== "update_scene");
